...
### Your code here
Trips %>%
  filter(as.numeric(edate - sdate) < 3600) %>%
...
### Your Code Here
Trips %>%
  ggplot(aes(x=lubridate::yday(sdate))) + geom_histogram()
```


From the histogram we can observe that, because the data is collected in specific time of the year, day of the year is from 275 to 365. The count (density) of bike checkouts gradually decreases from 275 to 365. Top checkouts are somewhere around day 280.
...
### Your Code Here
Trips %>%
  ggplot(aes(x=lubridate::wday(sdate))) + geom_histogram()
```


From the histogram we can see that the cont slightly increases as we get closer to weekends and the top checkout is on Thursday. Starting from Thursday, the amount of checkouts start decreasing.
...
### Your Code Here
Trips %>%
  ggplot(aes(x=lubridate::hour(sdate))) + geom_histogram()

```


According to the histogram, between 0 and 5 am there are very few bike checkouts. The spike starts around 6 am and lasts till around 9-10 pm. The most bike checkouts are around 5 pm.
...
### Your Code Here
Trips %>%
  ggplot(aes(x=lubridate::minute(sdate))) + geom_histogram()
```


From the histogram we can see that it is kind of stable. There are really no positive or negative trends when it comes to bike checkouts based on the minute of the hour.
...
### My Code here
Trips %>%
    group_by(hourOfTheDay = lubridate::hour(sdate), dayOfTheWeek = lubridate::wday(sdate), client) %>%
    dplyr::summarise(count=n()) %>%
    ggplot(aes(x=hourOfTheDay,y=count)) +
    geom_point(aes(col = client, )) + facet_wrap(~dayOfTheWeek)
```

The most checkouts occur between 10am and 8pm and the majority of the people who checkout with bikes are resgistered clients. 
...
Simple2 <-
  Simple %>%
  dplyr::rename(estation = sstation, lat2 = lat, long2 = long)
```
...
```
The graphs of trip distance densities and distance between stations is similar. Because people only can leave bikes at the stations, the combination of distance of travel from one station to another should be similar to the combination of pairs of stations. 
...
Because the point of merging Simple and Simple 2 was to get the combination of stations with their coordinates for the further distance calculation. Thus, we have to give different variable names for stations so one is start and the other is end stations.
